...
flowchart TD
    ...
    S5 --> O12["KM_FigA_time_to_FIS_fullcohort.png/.pdf"]
    ...
    S10 --> O17["Fine-Gray outputs (6/12/24m):<br/>sHR tables, forests, CIF plots,<br/>TableS2 cohort derivation"]
...
| `5.KM1.R` | `master_clean_dataset1.rds` | Restricts to positive `Time_FIS_days`; derives `Time_FIS_years = Time_FIS_days / 365.25`; fits `survfit(Surv(Time_FIS_years, Status_FIS) ~ 1)`; plots cumulative incidence (1 - KM) of first inappropriate shock over 0-5 years with 95% CI and risk table; checks median estimability; reports cumulative incidence at 1/3/5 years; explicitly descriptive (no time-dependent exposure handling) | `KM_FigA_time_to_FIS_fullcohort.png/.pdf` |
...
| `10.Fine_gray.R` | `Imputed_data/mice_full_object1.rds` | Competing-risk sensitivity using `Survival_time` (months, converted with 30.4375 days/month) and `Status` (0 = censored, 1 = appropriate shock = event of interest, 2 = death as competing event); landmarks 183/365/731 days; excludes missing/zero `Survival_time`, treats missing `Status` as censored, reclassifies `Time_FIS_days > time_days` as unexposed (Rule E); same landmark restriction and `FIS_L` derivation as script 9; fits `cmprsk::crr(..., failcode = 1)` with the landmark adjustment set but NYHA binarized and no `strata(DB)` (unsupported by `crr()`); pools across imputations with manual Rubin's rules; CIF plots from imputation 1 | Per landmark: `FULL_sHR_table_<lm>.pdf`, `FULL_forest_<lm>.png/.pdf`, `FULL_CIF_<lm>.png/.pdf`; `TableS2_FineGray_CohortDerivation.html` |
...
| `Survival_time`, `Status` | Merged cohort (used in script 10) | Fine-Gray outcome: `Survival_time` in months (converted with 30.4375 days/month); `Status` coded 0 = censored/alive, 1 = appropriate shock (event of interest), 2 = death (competing event). Distinct from the `Time_death_days`/`Status_death` pair used in Cox/KM analyses. |
